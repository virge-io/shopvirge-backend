...
per include.
    ...
    authenticated   require_cognito               collection management
    admin           require_cognito, require_admin cross-shop admin views
    shop            require_shop                  per-shop, API key or Cognito
    shop_cognito    require_cognito, require_shop  per-shop, keys refused
...
``shops.shop_router`` (``/shops/{shop_id}``, ``/shops/config/{shop_id}``,
``/shops/allowed-ips/{shop_id}``) and ``orders.per_shop_router``
(``/orders/shop/{shop_id}``). The orders merge removes the latter.
...
(``GET /shops/{shop_id}``). ``test_router_posture.py`` asserts no route is shadowed.
"""
...
shop_cognito.include_router(accounts.shop_router)
shop_cognito.include_router(attributes.shop_router)

shop.include_router(categories.router)
shop.include_router(products.router)
shop.include_router(revisions.router)
shop.include_router(tags.router)
shop.include_router(attributes.router)
...
api_router.include_router(shop_cognito)
